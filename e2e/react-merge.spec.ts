...
const mergePageFile = path.join(pagesDir, 'merge-e2e.tsx')
const indexPageFile = path.join(pagesDir, 'index.tsx')
...
/** Wait until routes.ts matches predicate and content stops changing (dev watch may write twice). */
...
async function resetRoutesFromPages() {
  ...
  await waitForRoutesStable(
    (text) =>
      text.includes('./pages/about.tsx')
      && !text.includes('e2eMerge')
      && !text.includes('./pages/merge-e2e.tsx'),
  )
}
...
export default function MergeE2E() {
  return (
    <div data-testid="merge-e2e-page">
      <DemoPage
        title="Merge E2E"
        lead="Added at runtime to verify merge under Vite watch."
...
`,
  ...
  return waitForRoutesStable(
    (text) => text.includes('e2eMerge: "keep-about"') && text.includes('./pages/merge-e2e.tsx'),
  )
}

test.describe('merge hot update', () => {
  ...
  test('preserves hand-edited about route when a new page is added under dev watch', async () => {
  ...
  test('navigates merged routes in the browser after routes.ts regen', async ({ page }) => {
    await waitForDevUrl('http://localhost:5199/')
    await expect(async () => {
    ...
    await waitForDevUrl('http://localhost:5199/merge-e2e')
